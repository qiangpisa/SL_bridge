...
def load_IC_data(dataset_name, used_ratio=1., source_path='../graphdiffdata/'):
    ...
    if raw_data_dict['inverse_pair'].ndim > 3:
        raw_data_dict['inverse_pair'] = raw_data_dict['inverse_pair'].reshape(-1,
                                                                              *raw_data_dict['inverse_pair'].shape[-2:])
    raw_data_dict['inverse_pair'] = raw_data_dict['inverse_pair'][
                                    :int(used_ratio * raw_data_dict['inverse_pair'].shape[0])]
...
def load_data(dataset_name, dataset_path='../graphdiffdata', used_ratio=1., not_latent=False):
    dataIC_train = ICDataset(dataset_name, mode='train', dataset_path=dataset_path, used_ratio=used_ratio,
                             not_latent=not_latent)
    dataIC_valid = ICDataset(dataset_name, mode='valid', dataset_path=dataset_path, used_ratio=used_ratio,
                             not_latent=not_latent)
    dataIC_test = ICDataset(dataset_name, mode='test', dataset_path=dataset_path, used_ratio=used_ratio,
                            not_latent=not_latent)
    # dataloader
...
class ICDataset(DGLDataset):
    ...
    def __init__(self, dataset_name, mode, dataset_path='../graphdiffdata/', used_ratio=1., train_ratio=0.75,
                 valid_ratio=0.125, not_latent=False):
        ...
        self.raw_data_dict, self.sample_num, self.node_num = load_IC_data(dataset_name, used_ratio=self.used_ratio,
    ...
    def process(self):
        ...
        if self.one_hot_label:
            # to one-hot
            label = np.eye(data_dict['inverse_pair'].shape[-1])[label.astype(int)].reshape(-1, data_dict[
                'inverse_pair'].shape[-1])
            feat = np.eye(data_dict['inverse_pair'].shape[-1])[feat.astype(int)].reshape(-1, data_dict[
                'inverse_pair'].shape[-1])
